...
srun python -m train -data /data/nssd_data/copy_net_dataset/$DATA_NAME/$DATA_NAME -vocab /data/nssd_data/copy_net_dataset/$DATA_NAME/$DATA_NAME.vocab.pt -exp_path "/tmp/seq2seq-keyphrase-pytorch/exp/$EXP_NAME/%s.%s" -model_path "/tmp/seq2seq-keyphrase-pytorch/model/$EXP_NAME/%s.%s" -pred_path "/tmp/seq2seq-keyphrase-pytorch/pred/$EXP_NAME/%s.%s" -exp "$DATA_NAME" -batch_size 128 -bidirectional -copy_model -run_valid_every 2000 -save_model_every 10000 -beam_size 16 -beam_search_batch_size 32 -train_ml -attention_mode $ATTENTION -input_feeding # -must_teacher_forcing
